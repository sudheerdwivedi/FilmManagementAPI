...
public class AuthChannelInterceptor implements ChannelInterceptor {
	...
	public Message<?> preSend(Message<?> message, MessageChannel channel) {
		...
		if (accessor.getCommand() == StompCommand.CONNECT) {
			...
			System.out.println("::::::Come to PreSend:::::"+username);
			// retrieve the password from the headers
			final String password = accessor.getFirstNativeHeader(PASSWORD_HEADER);
			System.out.println("::::::Come to PreSend:::::"+password);
			// authenticate the user and if that's successful add their user information to
			// the headers.
